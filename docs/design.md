...
flowchart LR
  ...
  User --> Command
  Command --> Feature[交換可能な機能]
  Feature --> Greeting[現在のサンプル: 挨拶]
...
`src/greeting`の挨拶出力は、CLI基盤の必須要素ではありません。この機能は、引数解析、入力検査、機能の呼び出し、標準出力、テストの組み合わせを確認するためのサンプルです。実際のCLIでは、`src/greeting`を製品固有の機能へ置き換えます。
